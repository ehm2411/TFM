...
qq_hba1c          <- ggqqplot(datos$hba1c, main = "QQ-plot para HBA1C") + theme(plot.title = element_text(hjust = 0.5))
qq_hba1c2          <- ggqqplot((datos$hba1c)**2, main = "QQ-plot para HBA1C ^2") + theme(plot.title = element_text(hjust = 0.5))
qq_hba1c3          <- ggqqplot(sqrt(datos$hba1c), main = "QQ-plot para sqrt(HBA1C)") + theme(plot.title = element_text(hjust = 0.5))
qq_hba1c4          <- ggqqplot(log(datos$hba1c), main = "QQ-plot para log(HBA1C)") + theme(plot.title = element_text(hjust = 0.5))

grid.arrange(qq_hba1c,qq_hba1c2,qq_hba1c3,qq_hba1c4, ncol=2)

qq_age            <- ggqqplot(datos$age, main = "QQ-plot para EDAD") + theme(plot.title = element_text(hjust = 0.5))
qq_dmdura         <- ggqqplot(datos$dmdura, main = "QQ-plot para DMDURA") + theme(plot.title = element_text(hjust = 0.5))
qq_ldl            <- ggqqplot(datos$ldl, main = "QQ-plot para LDH") + theme(plot.title = element_text(hjust = 0.5))
qq_hdl            <- ggqqplot(datos$hdl, main = "QQ-plot para HDL") + theme(plot.title = element_text(hjust = 0.5))
qq_trig           <- ggqqplot(datos$trig, main = "QQ-plot para TRIG") + theme(plot.title = element_text(hjust = 0.5))
qq_sbp            <- ggqqplot(datos$sbp, main = "QQ-plot para SBP") + theme(plot.title = element_text(hjust = 0.5))
qq_dbp            <- ggqqplot(datos$dbp, main = "QQ-plot para DBP") + theme(plot.title = element_text(hjust = 0.5))
qq_DMSES.Diet     <- ggqqplot(datos$DMSES.Diet, main = "QQ-plot para DMSES Diet") + theme(plot.title = element_text(hjust = 0.5))
qq_DMSES.Monitor  <- ggqqplot(datos$DMSES.Monitor, main = "QQ-plot para DMSES Monitor") + theme(plot.title = element_text(hjust = 0.5))
qq_DMSES.Physical <- ggqqplot(datos$DMSES.Physical, main = "QQ-plot para DMSES Physical") + theme(plot.title = element_text(hjust = 0.5))
qq_DMSES.Regimen  <- ggqqplot(datos$DMSES.Regimen, main = "QQ-plot para DMSES Regimen") + theme(plot.title = element_text(hjust = 0.5))
qq_DMSES.Total    <- ggqqplot(datos$DMSES.Total, main = "QQ-plot para DMSES Total") + theme(plot.title = element_text(hjust = 0.5))
qq_DK.10          <- ggqqplot(datos$DK.10, main = "QQ-plot para DK.10") + theme(plot.title = element_text(hjust = 0.5))
qq_bmi            <- ggqqplot(datos$bmi, main = "QQ-plot para BMI") + theme(plot.title = element_text(hjust = 0.5))
...
Tenemos un dependencia entre las variables alerta y hospital, estado civil, ingresos, enfermedad coronaria, enfermedad renal, tipo de tratamiento médico d ela diabetes, complicación de la diabetes, problemas miocardio, insuficiancia renal, nefropatía y  retinopatía.
...
require(leaps)
b <- regsubsets(hba1c~.,data=datos1,nvmax = 15)
rs <- summary(b)
rs$which
...
Los factores de inflación de la varianza se mantienten bajos en los modelos analizados.
...
Añadir más variables no mejora sustancialmente el modelo logístico, y sólo con la puntuación DMSES.Monitor ya se tenía un buen resultado.
...
Añadir más variables no mejora sustancialmente el modelo logístico, y sólo con la puntuación DMSES.Regimen ya se tenía un buen resultado.
...
Añadir más variables no mejora sustancialmente el modelo logístico, y sólo con la puntuación DMSES.Physical ya se tenía un buen resultado.
...
```{r clu_4, eval=TRUE, warning= FALSE ,cache=TRUE}
